...
public class Interpreter extends MiniJava  {
	...
	public static final int STH=23;
	public static final int ALLOCH=24;
		...
		for(int ic=0; ic<program.length&&!halt; ic++) {
			...
			case ALLOCH:
				...
				if(heap[heap.length-1]==heap.length-2) {
					...
					lastaddr=heap[heap.length-1]-1;
					begin=(heap[lastaddr]&0xFFFF);
					end=begin+size;
				}
				heap[heap[heap.length-1]]=begin<<16|end;
				heap[heap.length-1]--;
				if(begin>heap[heap.length-1])
					error("Heap full\n");
				push(begin);
				break;
			default:
